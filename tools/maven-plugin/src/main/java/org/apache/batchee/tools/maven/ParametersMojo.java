...
public class ParametersMojo extends BatchEEMojoBase {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        final Properties parameters = getOrCreateOperator().getParameters(executionId);
        getLog().info("Job parameters for execution #" + executionId + ":");
        for (final String key : parameters.stringPropertyNames()) {
            getLog().info(" - " + key + " = " + parameters.getProperty(key));
        }
    }
}
